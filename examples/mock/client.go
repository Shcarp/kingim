...
func (d *WebsocketDialer) DialAndHandshake(ctx kingim.DialerContext) (net.Conn, error) {
	// 1 调用ws.Dial拨号
...
func (d *TCPDialer) DialAndHandshake(ctx kingim.DialerContext) (net.Conn, error) {
	...
	// 2. 发送用户认证信息，示例就是userid
	err = tcp.WriteFrame(conn, kingim.OpBinary, []byte(ctx.Id))
	if err != nil {
		return nil, err
	}
	// 3. return conn
