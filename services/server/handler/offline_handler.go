...
func NewOfflineHandler(message service.Message) *OfflineHandler {
	return &OfflineHandler{msgService: message}
}

func (h*OfflineHandler) DoSyncIndex(ctx kingim.Context) {
...
func (h*OfflineHandler) DoSyncContent (ctx kingim.Context) {
	...
	if len(req.MessageIds) == 0 {
		_ = ctx.RespWithError(pkt.Status_InvalidPacketBody, errors.New("messageIds is empty"))
		return
	}
